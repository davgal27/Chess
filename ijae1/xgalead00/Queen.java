...
public class Queen extends Piece {
    ...
    public boolean CanMoveTo(int nextrow, int nextcol, Piece[][] ChessBoard) {
        ...
        if (!((Math.abs(row - nextrow) == Math.abs(col - nextcol)) || // ensures diagonal movement
            ...
            JOptionPane.showMessageDialog(null, "Illegal Move for Queen! Queen can only move diagonally, horizontally, or vertically!");
            return false;
        }
        // Calculate steps
        ...
        while (currentrow != nextrow || currentcol != nextcol){
            if (ChessBoard[currentrow][currentcol] != null) {
                JOptionPane.showMessageDialog(null, "Obstruction! Illegal move for Queen!");
                return false;
            }
            currentrow += rowstep;
            currentcol += colstep;
